function [rec,dist,PSNR,SNR, ac_decodeSignal] = Wavelet_processing(signal,Bits_length,fs)
...
ac_decodeSignal = AC_Dec(ac_encodeSignal);


%% Decoding
rec  = Decoder(EncodeSignal);
dist = mse(psignal(1:length(signal)),rec(1:length(signal)));
PSNR = 10*log10((length(signal)*6^2)/(sum((rec(1:length(signal))-psignal(1:length(signal))).^2)));
SNR  = 10*log10(sum((psignal(1:length(signal))).^2)/(sum((rec(1:length(signal))-psignal(1:length(signal))).^2)));
